...
export default function AnalyticsPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F2F4EF' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: isMobile ? '24px 16px 48px' : '32px 24px 64px' }}>

        <div style={{ marginBottom: 32 }}>
          <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: MUTED, letterSpacing: '0.12em', marginBottom: 8 }}>ANALYTICS</div>
          ...
          <p style={{ fontFamily: 'IBM Plex Sans, sans-serif', fontSize: 13, color: MUTED, margin: 0 }}>
            Usage trends and district-level risk comparison
          </p>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 12, marginBottom: 24 }}>
          {[
            { label: 'Total Queries', value: String(totalAllTime), sub: 'all time', color: '#1F6F64' },
            { label: 'Peak Day (7d)', value: peakDay ? String(peakDay.totalQueries) : '--', sub: peakDay?.date || 'no data yet', color: '#1F6F64' },
            { label: 'Avg. Critical/Day', value: avgCritical, sub: 'last 7 days', color: '#B54A2A' },
            { label: 'Districts Covered', value: String(ranking.length), sub: 'of 14 total', color: '#7C9A3C' },
          ].map(card => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.2fr 1fr', gap: 16, marginBottom: 16 }}>
          <div style={{ ...CARD, padding: '20px 24px' }}>
            <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: MUTED, letterSpacing: '0.1em', marginBottom: 20 }}>RISK ALERT TREND — 7 DAYS</div>
            {!historyLoading && history.length === 0 ? (
              <p style={{ fontFamily: 'IBM Plex Sans, sans-serif', fontSize: 13, color: MUTED, textAlign: 'center', padding: '40px 0' }}>
                No activity recorded yet. This chart fills in as searches happen.
              </p>
            ) : (
          ...
          <div style={{ ...CARD, padding: '20px 24px' }}>
            <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: MUTED, letterSpacing: '0.1em', marginBottom: 20 }}>QUERY VOLUME — 7 DAYS</div>
            {!historyLoading && history.length === 0 ? (
              <p style={{ fontFamily: 'IBM Plex Sans, sans-serif', fontSize: 13, color: MUTED, textAlign: 'center', padding: '40px 0' }}>
        ...
        <div style={{ ...CARD, padding: isMobile ? '20px 16px' : '20px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexWrap: 'wrap' as const, gap: 8 }}>
            <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: MUTED, letterSpacing: '0.1em' }}>DISTRICT RISK RANKING</div>
          ...
          <p style={{ fontFamily: 'IBM Plex Sans, sans-serif', fontSize: 11, color: MUTED, marginBottom: 16 }}>
            Ranked by combined flood and landslide risk across sampled locations in each district.
          </p>

          {rankingLoading && <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, color: MUTED }}>Loading district rankings…</p>}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {ranking.map((d, i) => {
              ...
              return (
                <div key={d.district} style={{ display: 'grid', gridTemplateColumns: isMobile ? '20px 90px 1fr 50px' : '24px 130px 1fr 60px', alignItems: 'center', gap: isMobile ? 8 : 12 }}>
                  <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: MUTED, textAlign: 'right' }}>{String(i + 1).padStart(2, '0')}</span>
                  <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 600, fontSize: isMobile ? 11 : 13, color: '#12262B', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.district}</span>
