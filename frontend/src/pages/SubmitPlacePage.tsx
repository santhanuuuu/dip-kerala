...
const placeTypes = [
  ...
  { value: 'local_area', label: 'Local Area / Locality' },
];
...
export default function SubmitPlacePage({ isLoggedIn }: { isLoggedIn: boolean }) {
      ...
      <div style={{ minHeight: '60vh', background: '#F2F4EF', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        <div style={{ textAlign: 'center', padding: '48px 40px', background: '#ffffff', border: '1px solid rgba(124,154,60,0.25)', borderTop: '3px solid #7C9A3C', borderRadius: 3, maxWidth: 480, boxShadow: '0 4px 16px rgba(18,38,43,0.08)' }}>
          ...
          <div style={{ fontFamily: 'IBM Plex Sans, sans-serif', fontSize: 14, color: MUTED, lineHeight: 1.6, marginBottom: 20 }}>
            <strong style={{ color: '#12262B' }}>{form.name}</strong> has been submitted for review. We'll verify the details and add it to the map once approved.
          </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F2F4EF' }}>
      <div style={{ maxWidth: 680, margin: '0 auto', padding: isMobile ? '24px 16px 48px' : '32px 24px 64px' }}>
        <div style={{ marginBottom: 32 }}>
          <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: MUTED, letterSpacing: '0.12em', marginBottom: 8 }}>SUBMIT A PLACE</div>
          ...
          <p style={{ fontFamily: 'IBM Plex Sans, sans-serif', fontSize: 13, color: MUTED, margin: 0 }}>
            Can't find your panchayat, municipality, city, or local area on the map? Add it here and we'll get it added.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ background: '#ffffff', border: '1px solid rgba(18,38,43,0.09)', borderRadius: 2, padding: isMobile ? '20px 18px' : '24px 28px', display: 'flex', flexDirection: 'column', gap: 18, boxShadow: '0 2px 8px rgba(18,38,43,0.05)' }}>
            <div>
